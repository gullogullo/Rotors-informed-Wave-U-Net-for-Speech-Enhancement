...
class BaseTrainer:
    def __init__(self,
        ...
        self.scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.1, patience=10) #15
        ...
        self.early_stopping = EarlyStopping(tolerance=10, min_delta=0) #30
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            loss_train = self._train_epoch(epoch)
            ...
            if self.validation_interval != 0 and epoch % self.validation_interval == 0:
                ...
                score, loss_valid = self._validation_epoch(epoch)
                print('SCORE: ', score)
                print('VALID LOSS: ', loss_valid)
                self.scheduler.step(loss_valid)
                if loss_valid <= previous_loss_valid: # score <= previous_loss_valid
                    previous_loss_valid = loss_valid

                if self._is_best(loss_valid, find_max=self.find_max):
                    self._save_checkpoint(epoch, is_best=True)
                    best_epoch = epoch

                self.early_stopping(previous_loss_valid, loss_valid)
...
class EarlyStopping:
    ...
    def __call__(self, previous_loss, current_loss):
        if (current_loss - previous_loss) > self.min_delta:
            self.counter +=1
