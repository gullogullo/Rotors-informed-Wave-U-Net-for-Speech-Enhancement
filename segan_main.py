...
from segan_data_preprocess import sample_rate
from segan_model import Generator, Discriminator
from util.utils import AudioDataset, emphasis
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_epochs_valid', default=10, type=int, help='epochs number for validation')

    opt = parser.parse_args()
    BATCH_SIZE = opt.batch_size
    NUM_EPOCHS = opt.num_epochs
    NUM_EPOCH_VALID = opt.num_epochs_valid
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        noisy_loss_total = 0.0
        g_loss_total = 0.0
        g_cond_loss_total = 0.0
            ...
            for train_batch, train_clean, train_noisy in train_bar:
                ...
                train_bar.set_description(
                    'Epoch {}-{}: d_clean_loss {:.4f}, d_noisy_loss {:.4f}, g_loss {:.4f}, g_conditional_loss {:.4f}'
                        .format(epoch + 1, n, clean_loss.item(), noisy_loss.item(), g_loss.item(), g_cond_loss.item()))
                clean_loss_total += clean_loss.item()
                noisy_loss_total += noisy_loss.item()
                g_loss_total += g_loss.item()
                g_cond_loss_total += g_cond_loss.item()
        print('Epoch {}: d_clean_loss {:.4f}, d_noisy_loss {:.4f}, g_loss {:.4f}, g_conditional_loss {:.4f}'
                        .format(epoch + 1, clean_loss_total, noisy_loss_total, g_loss_total, g_cond_loss_total))
                
            

        # TEST model
        if ((epoch + 1) % NUM_EPOCH_VALID) == 0:
            ...
            # save the model parameters every validation interval
